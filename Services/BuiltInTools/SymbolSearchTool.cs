...
namespace DeepSeek_v4_for_VisualStudio.Services.BuiltInTools
{
    ...
    public class SymbolSearchTool : BuiltInToolBase
    {
        ...
        public override async Task<string> ExecuteAsync(Dictionary<string, JsonElement> args, string? workspaceRoot)
        {
            ...
            try
            {
                // ── IVsFindSymbol2 是 COM 对象，必须在 UI 线程调用 ──
                await ThreadHelper.JoinableTaskFactory.SwitchToMainThreadAsync(CancellationToken);

                var objSearch = await ServiceProvider.GetGlobalServiceAsync(
                    typeof(SVsObjectSearch));

                if (objSearch is not IVsFindSymbol2 findSymbol)
                    return LocalizationService.Instance["tool.symbolSearch.serviceUnavailable"];
                ...
                // ── 执行搜索 ──
                int hr = findSymbol.FindSymbol2(query, flags, out IVsEnumSymbolPaths enumSymbols);

                if (hr != VSConstants.S_OK || enumSymbols == null)
                    return LocalizationService.Instance.Format("tool.symbolSearch.noResults", query);
                ...
                {
                    hr = enumSymbols.Next(1, pathArray, out fetched);
